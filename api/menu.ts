...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const [catRes, itemsRes, settingsRes] = await Promise.all([
      ...
      fetch(`${url}/rest/v1/menu_settings?id=eq.1`, { headers }),
    ]);
    ...
    const grouped = categories.map((cat: any) => ({
      id: cat.id, name: cat.name, slug: cat.slug, side: cat.side, sort_order: cat.sort_order,
      attribution: cat.attribution || null,
      items: items.filter((item: any) => item.category_id === cat.id).map((item: any) => ({
        ...
        available: item.available, sort_order: item.sort_order,
    ...
    return res.status(200).json({ categories: grouped, settings });
  } catch (err) {
